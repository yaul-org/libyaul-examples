    ...
    void* buffer, uint32_t length)
{
        ...
        if(fileCursor->cursor) {
                ...
                if(missingBytes <= remainingBuffer) {
                        ...
                        /* If remainingBuffer is 0, then this memcopy does
                         * nothing and we prepare for a normal read from a fresh
                         * sector */
...
uint32_t
getFilenameHash(const char *filename, uint32_t length)
{
        return generateHash(filename, length, 0, HASH_PRIME, HASH_PRIME, NULL);
}
